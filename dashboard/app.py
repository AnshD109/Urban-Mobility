"""
dashboard/app.py — Standalone Version
Loads .joblib models directly — no FastAPI services needed.
...
if page == "📊 ETA & Pricing":
    ...
    if st.button("🚀 Get Quote", type="primary"):
        if "eta_rf" not in models or "feature_pipeline" not in models:
            ...
            try:
                # Build feature dict matching your feature_pipeline expectations
                ...
                # Transform with feature pipeline
                X = models["feature_pipeline"].transform(raw)

                # Predict ETA — median model
                eta_p50 = float(models["eta_rf"].predict(X)[0])

                # Quantile bounds (if quantile model available)
                ...
                st.caption("This usually means the feature names in the input don't match what the pipeline expects. Check that your feature_pipeline.joblib was saved with the same column names.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("🔍 Assess Risk", type="primary"):
        if "maintenance_rf" not in models:
            st.error("❌ Maintenance model not found. Make sure models/ folder is in your repo.")
        else:
            try:
                ...
                if risk_score < 25:
                    level, icon, rec = "LOW",      "🟢", "No immediate action required. Schedule routine inspection in 3 months."
                elif risk_score < 50:
                    level, icon, rec = "MEDIUM",   "🟡", "Monitor closely. Schedule preventive maintenance within 30 days."
                elif risk_score < 75:
                    level, icon, rec = "HIGH",     "🟠", "Schedule maintenance within 7 days. Check engine and battery systems."
                else:
                ...
                ax.barh(["Risk"], [100 - risk_score], left=risk_score,
                ...
                st.error(f"❌ Prediction error: {e}")
                st.caption(f"Feature mismatch or model error: {e}")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("Classify incident or customer feedback text into actionable categories.")
    ...
    if st.button("🔍 Classify", type="primary") and text.strip():
        if "nlp_pipeline" not in models:
            st.error("❌ NLP model not found. Make sure models/ folder is in your repo.")
        else:
            try:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("- Evening rush (17–19h) drives highest volume and longest average duration.")
    st.markdown("- Early morning (2–5h) has lowest volume but fastest trips.")
    ...
    st.dataframe(model_results.style.highlight_min(subset=["MAE (min)"], color="#c8f7c5")
